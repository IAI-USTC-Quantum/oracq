...
\item \textbf{Solver certification is small-scale.}  All six ODE families,
  ...
  defects found by the campaign are recorded but not yet repaired.  None is
...
\item \textbf{No resource estimator, no error budgeting}---scoped in
  \secref{sec:reserved}.
\end{itemize}
...
injection point; robust and maximum-likelihood amplitude estimation; a
...
verifies its own synthesis---is, to our knowledge, unmatched by existing
quantum programming systems; its verification discipline is deliberately
graduated, and its most important open axis, resource estimation and
solver-level certification, is reserved rather than overclaimed.  We offer
