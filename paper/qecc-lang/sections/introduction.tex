...
\item \textbf{Programs are deep, symbolic, and large.}  Scientific-computing
  ...
  $\rightarrow$ readout); by the time a solver is assembled, the gate list is
  already far too deep to review, diff, or serialize as flat text.  What is
  needed is an intermediate
...
\item \textbf{A requirements analysis} that derives, from the structure of
  ...
  preservation, capability tracking, symbolic structure, promise provenance,
...
\item \textbf{A language design} meeting these requirements: an embedded
  ...
  with an open-module oracle system, a linking pass, capability checking,
  and a canonical JSON serialization with schema---executed natively by a
  register-level RIR interpreter contributed to PySparQ
...
\item \textbf{A numerical verification campaign on real backends}
  ...
  and surfaced two genuine defects, both now repaired and pinned by
  regression tests.  What remains open is the parameter-scaling
  axis---tracked explicitly rather than overclaimed.
\item \textbf{A compositional resource estimator}
  (Appendix~\ref{sec:resources}): a strict Toffoli+Clifford+T+QRAM compile
  ...
  metric.  We present this as a secondary capability enabled by the same
  structure preservation; the scaling study, the query-count view of QRAM,
  and two lowering gaps exposed by the accounting appear in the appendix.
\end{itemize}
...
defects, a brief summary of the resource-estimation capability (detailed in
Appendix~\ref{sec:resources}), and
