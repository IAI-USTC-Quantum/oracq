...
print(simulate(program).amplitudes)      # {(0,): 1, (3,): 1} / sqrt 2
...
(\texttt{add\_const}), QRAM \texttt{Load}, and a restricted classical-memory
\texttt{Store}. All richer
...
Control, repetition, and inversion are structured blocks, entered as Python
context managers.  A \texttt{Repeat} node carries a static count and its body
exactly once; the body must be structurally
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}llp{9.6cm}@{}}
    ...
    \texttt{Store} & instruction & classical memory update at definite address/data values; forbidden in controlled or adjoint contexts \\
    ...
    \texttt{Repeat} & instruction & static count; body preserved, never replicated \\
...
\item \textbf{Explicit operational boundary.} RIR represents operations after compile-time
  ...
  (\secref{sec:readout}). The restricted Store effect updates classical
  memory and excludes its containing modules from adjoint and control.
\item \textbf{Canonical, hardened serialization.}  Programs serialize to
  ...
  programs remain readable by newer versions of the framework.
...
Appendix~\ref{app:spec} condenses both machine-facing formats for
reference: an EBNF grammar of the RIR object model together with its JSON
encoding rules and a canonical worked example, and the grammar of the
OriginIR-ext dialect together with the lowering rules that produce it from
RIR.
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[
    ...
    \node[phase] at (-7.5, 2.7) {generate};
    \node[phase] at (-7.5, -1.6) {store \& link};
    \node[phase] at (-7.5, -5.45) {export / execute};
    \draw[sepline] (-7.0, -0.5) -- (7.15, -0.5);
    \draw[sepline] (-7.0, -4.05) -- (7.15, -4.05);
    ...
    \draw[arr] (bindbox.south) to[bend left=14] (rir.south east);
    \draw[arr] (rir) -- (json);
