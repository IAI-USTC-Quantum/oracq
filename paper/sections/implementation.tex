...
This section distinguishes implementation status, instance-level
numerical evidence, and broader mathematical claims. Results are scoped
by program configuration, reference semantics, and backend environment.
The previously recorded numerical campaign is a snapshot of its tested
revision, not automatic validation of later changes.

For the workflow revision described here, the core/schema suite passes
391 tests and 278 subtests, and the real-backend integration suite passes
41 tests. The oracle, block-encoding, arithmetic, numerical-function,
QHAM/QFVM, and ODE verification groups were rerun, comprising 193 cases.
The nine realization-study configurations of
Section~\ref{sec:oracle-study} were additionally checked on all four
execution paths. The earlier thirteen-group campaign below is retained
as a separately scoped record.
...
\subsection{Complementary verification evidence}
\label{sec:evidence-ladder}

Verification records use the following categories from the project's
test plan. Their historical L0--L5 labels are retained for traceability;
they are complementary checks, not a total ordering of mathematical
confidence:
...
What the ladder does \emph{not} by itself include is numerical validation
...
\begin{table}[t]
  ...
  amplitude agreement of assembled circuits; ``numerical validation'' =
  ...
  \setlength{\tabcolsep}{4pt}\begin{tabular}{@{}p{4.3cm}cccc@{}}
    ...
    \bottomrule
    \multicolumn{5}{@{}p{14.3cm}@{}}{\scriptsize $^a$QSP synthesis self-verifies polynomial round-trips; transforms are checked end-to-end in \secref{sec:numverify}.  $^b$assembled-circuit agreement on real backends.  $^c$all six families checked against truncated-oracle references at small scale; the Schr\"odingerization momentum-sign defect found by an earlier campaign round is repaired and pinned by regression cases (\secref{sec:numverify}); success channels and large-scale accuracy remain pending.  $^d$classical flux and flow-state witnesses.  $^e$fixed-point physics checked against an independent fixed-point semantics at \texttt{FixedFormat(5,2)}; generation-time constant quantization dominates the method error in coarser formats.  $^f$hand-computed counts and atom-by-atom cross-checks against the strict netlist; scaling expectations validated in Appendix~\ref{sec:resources}.}
  \end{tabular}
\end{table}
...
We close the validation column at the scale where exact classical
...
an independently written fixed-point semantics. Cases use basis-input
sweeps and selected superposition tests, with coverage determined by each
case's construction; a uniform input alone does not characterize an
arbitrary oracle. Where an algorithm combines an exact construction
...
The scope of this validation is deliberately register-exact: instances
...
\subsection{Resource analysis before and after binding}
\label{sec:resources-summary}

Resource analysis is part of the algorithm-development workflow.
When closure checking is disabled, the estimator returns the known
logical costs and a ledger of unresolved oracle calls, distinguished by
slot, control count, adjoint context, and resource arguments.
Conceptually, additive resources have the form
\[
 C(P)=C_{\mathrm{known}}(P)+\sum_{s,v} N_{s,v}\,C_{s,v},
\]
where $v$ specifies the call variant. The unknown terms are reported,
not replaced by zero. Qubit workspace is a peak-live quantity and is
handled separately: an open report gives a known lower bound and lists
the unresolved workspace contributions rather than a complete qubit count.

After binding, the same estimator follows the concrete implementations.
Resource parameters are substituted on every call edge, so two calls of
one lookup module against different memories remain separate in the
entry-level ledger. Rotations are stored by angle and multiplicity;
even a rotation inside a \texttt{Repeat} of $2^{40}$ is counted without
materializing its occurrences. The JSON report exposes the remaining
oracle terms, completeness status, resource-specific reads and writes,
and the per-rotation synthesis-error setting.
...
$T$, and QRAM queries per (module, control-count, adjoint) variant,
multiplying through \texttt{Repeat} nodes without expansion. This logical
analysis supports the realization comparisons rather than introducing
a new physical resource model. The strict-gate-set compiler, the scaling study against theoretical
...
\item \textbf{Simulation campaigns for the solver stack at scale.}  The
  campaign of \secref{sec:numverify} checks implementations against
  ...
  systematic verification after further implementation changes.  The promise-provenance
...
numbers to an unchecked-at-scale solver stack would violate the very
