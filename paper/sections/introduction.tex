...
native register-level backends.
...
\item \textbf{An honest verification account}: a graded evidence ladder from
  static validation through real-backend circuit witnesses to (not yet
  performed) full numerical certification, with the status of every solver
  family recorded explicitly, and with backend, simulation, and resource
  estimation identified as the key open axis for future validation
...
describes implementation and verification status, including the deliberately
reserved parts (backend-level validation, simulation campaigns, and resource
estimation).  \secref{sec:discussion} discusses limitations and future work.
