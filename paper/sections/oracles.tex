...
signature, and choosing an interface role. Table~\ref{tab:paradigms}
lists nine built-in conventions; applications can use additional role
identifiers without changing a global registry or the RIR grammar.
These labels describe the intended interface, not a proof of its
mathematical interpretation.  The declaration optionally carries resources
...
\begin{table}[t]
  \centering
  \caption{The nine built-in oracle role conventions.  Each paradigm fixes the mathematical
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[
    ...
    \node[lbl] at (1.6, -3.15) {still open};
...
RIR also supports a restricted \texttt{Store}: address and data must each
have a definite computational-basis value at execution, and the
corresponding classical memory cell is updated without changing the
quantum registers. Superposed writes are rejected. A module containing
this effect, directly or through a call, cannot be controlled or inverted.
Writes are counted separately from quantum gates; this accounting
convention does not claim that physical memory updates are free.
...
Python structural interfaces, algorithm contracts, and RIR declarations
serve distinct boundaries. The canonical API names
\texttt{AlgorithmContract}, \texttt{QLSSSolver}, and \texttt{QODESolver}
identify checks and executable generators; the older names remain aliases.
An input provider may return a view backed by an open RIR module.
Contract resolution retains that actual view, and generation uses the
same resolved object. Calling a separate preflight check and then a solver
is two operations; no global provider cache is implied.
Unavailable methods, non-callable attributes, incompatible inspectable
signatures, and wrong return types receive structured diagnostics.
Exceptions inside a provider retain their original cause.

The \texttt{bind\_with\_report} entry point similarly returns both the
linked program and its report from one operation. The report includes
source and result program digests, resource mappings, unresolved slots,
and failures with their call paths. Captured-resource provenance survives
serialization, allowing later binding to reuse a memory and to maintain
a canonical order for newly captured arguments. These reports are
separate data artifacts, not Python callbacks embedded in the RIR.
...
three-argument \texttt{QODESolver} mapping (generator, initial state,
...
RIR keeps measurement outside the program (R7), so solver outputs can be
state oracles that further algorithms consume coherently. These solver
modules use the unitary fragment; the restricted classical-memory
\texttt{Store} effect does not admit controlled or adjoint composition.
