...
libraries that express fault-tolerant algorithms as composable units for
\emph{analysis} rather than for storage, linking, and execution.
Qualtran~\cite{harrigan2024qualtran} is the most direct structural
precedent for our IR: algorithms are trees of named \emph{bloqs} with
...
machinery), qubitization, and GQSP-based Hamiltonian simulation; it does
not carry open modules (every bloq is concrete), paradigm contracts,
promise provenance, or packaged solver families---there is no assembled
QLSS bloq and no ODE/PDE generators.
...
structure-preserving composition; none of them, however, treats the
\emph{program} as an open, re-linkable artifact, and none couples the
primitives to a solver-family protocol architecture.
...
level.  Across this landscape one pattern repeats: strong synthesis,
estimation, or domain workflows, but no system in which the scientific
algorithm is a stored, partially linked program whose promises travel
with it.
...
Table~\ref{tab:platform-comparison} sharpens the same boundary: Qualtran
and pyLIQTR pair serious primitive and resource machinery with concrete
program trees; Classiq resolves abstraction at synthesis time;
UnitaryLab ships solver products on a closed stack; Azure's estimator
prices any program's logical counts without asking how the program was
written.  None of them makes the \emph{open program}---partially bound,
contract-checked, promise-carrying---the unit of sharing.  To our
knowledge, no previously published system, open-source framework, or
commercial platform we surveyed combines these three units;
Tables~\ref{tab:qpl-comparison} and~\ref{tab:ode-coverage}
and~\ref{tab:platform-comparison} make the comparison concrete, and the honest
counterpoint is equally explicit: resource estimation---strong in Scaffold,
Q\# tooling, ProjectQ, Qualtran, pyLIQTR, and Azure's estimator---is the
deliberately reserved axis of
\secref{sec:reserved}.
