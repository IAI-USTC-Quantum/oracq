...
from ..library import BlockEncoding, _name, identity, product, scale, zero
from ..oracles import annotate, gate_state_prep, invoke, resources_for
from .quantum import PortBinding, QHAMBindings
from .reference import centered_weights
...
def term_encoding(discretization, term, *, max_coefficient_words=4096):
    ...
    coefficient = coefficient_encoding(discretization, monomial, max_words=max_coefficient_words)
...
def structured_fd_bindings(discretization, initial, *, max_coefficient_words=4096):
        ...
        encoded = lcu(
            [
                (
                    1,
                    term_encoding(
                        discretization, term, max_coefficient_words=max_coefficient_words
                    ),
                )
