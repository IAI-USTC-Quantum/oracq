...
from pyqecclang.infrastructure.ir import QRAM, Bits, ValidationError, fuse
...
class AdjacencyOracle(OracleView):
    ...
    def xor_database(self):
        ...
        from pyqecclang.algorithms.input_model.oracles import XorDatabase
...
def abstract_adjacency(vertex_bits, degree_bits, *, name=None) -> AdjacencyOracle:
...
def gate_adjacency(neighbors, *, name=None) -> AdjacencyOracle:
...
def qram_adjacency(vertex_bits, degree_bits, *, name=None) -> AdjacencyOracle:
...
def _neighbor_reflection(b, adjacency, prefix, vertex, peer, index):
...
def szegedy_walk(adjacency, *, name=None) -> Operation:
...
def szegedy_setup(adjacency, *, name=None) -> StatePreparation:
...
def quantum_walk_search(setup, walk, marked, steps, *, name=None) -> Operation:
...
def transition_matrix(neighbors):
...
def hitting_times(transition, marked):
...
def suggest_steps(transition, marked):
