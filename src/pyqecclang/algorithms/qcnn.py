...
def _kernel_value(kernel, spec, ki, kj, dd, q):
    kh, kw, d, dp = spec.kernel_shape
    return kernel[((ki * kw + kj) * d + dd) * dp + q]


def kernel_columns(kernel, spec):
...
class QCNNQRAM:
    ...
    def __init__(self, rows):
        ...
        self.tree_size = 2 * leaf_start
        self.trees = [[0.0] * self.tree_size for _ in range(self.count)]
        self.leaves = [[0.0] * self.width for _ in range(self.count)]
        for p, row in enumerate(rows):
            for r, value in enumerate(row):
                self._write(p, r, value)

    def _write(self, p, r, value):
