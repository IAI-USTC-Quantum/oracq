...
from pyqecclang.algorithms.input_model.oracles import invoke, qram_database, resources_for
...
def vector_angle_tables(rows, angle_width):
...
def _constant_register(builder, name, width, value):
...
def _prep_node_rotation(builder, angles, node_register, bit, angle_width):
...
def qcnn_vector_prep(count, vector_width, angle_width, *, name=None):
...
def qcnn_inner_product(row_count, column_count, vector_width, angle_width, *, name=None):
...
def quantized_prepared_state(vector, angle_width):
...
def qcnn_sampled_layer(x, kernel, spec, *, samples, eta, seed=0):
