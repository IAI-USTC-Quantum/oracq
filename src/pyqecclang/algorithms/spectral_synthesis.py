...
def _emit_ucr(builder, target, controls, thetas):
...
def uniformly_controlled_prep(amplitudes, *, name=None):
...
def merge_similar(terms):
    ...
    for coefficient, operand in terms:
        ...
        if key not in merged:
            ...
            order.append(key)
        merged[key][0] += complex(coefficient)
    return [(merged[key][0], merged[key][1]) for key in order if merged[key][0] != 0]


def fanout_spectral_diagonal(spectrum, width):
