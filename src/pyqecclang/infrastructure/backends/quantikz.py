...
class _Renderer:
    def __init__(self, program, wire_labels, name):
        self.program = program
        self.module = program.main
        ...
        self.row_of = {wire: row for row, wire in enumerate(self.wires)}
        self.columns = []
        self.groups = []

    def rows(self, ref: Ref):
        result = []
    ...
    def place_controls(self, column, target_rows, controls):
        top = min(target_rows)
        for row, value in controls:
            mark = "\\ctrl" if value else "\\octrl"
            column[row] = f"{mark}{{{top - row}}}"

    def box(self, rows, label, controls, *, dashed=False):
    ...
    def emit_primitive(self, node: Primitive, controls):
        op = node.op
        if op in _GATE_LABELS or op in _ROTATION_LABELS:
            label = _GATE_LABELS.get(op)
            if op in _ROTATION_LABELS:
                label = f"{_ROTATION_LABELS[op]}({_angle_text(node.angle)})"
            column = self.new_column()
        ...
        if op == "gphase":
            self.box(
                ...
                f"e^{{i{_angle_text(node.angle)}}}",
                controls,
            )
            return
        raise ValidationError(f"quantikz 暂不支持的门操作：{op}")

    def emit_load(self, node: Load, controls):
    ...
    def emit_grouped(self, node, controls, label):
...
def quantikz(source, *, wire_labels=True, name=None):
