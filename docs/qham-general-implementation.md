...
本轮将 QHAM 从固定 m=1 案例推进到规则驱动的生成器。先读 [数学推导](qham-general-derivation.md)，其后再看本文的实现与输入模型。依据是 [QHAM 论文 v2](https://arxiv.org/html/2411.06759v2)，正式论文题名为 Quantum homotopy analysis method with quantum-compatible linearization for nonlinear partial differential equations，[DOI](https://doi.org/10.1007/s11433-024-2584-2)。
...
入口 [pde.py](../src/pyqecclang/qham/pde.py) 将表达式拆成已知线性映射：
...
[linearization.py](../src/pyqecclang/qham/linearization.py) 自动生成：
...
格式见 [PDE Schema](pde.schema.json) 和 [QCL Schema](qcl-plan.schema.json)。PDE/QCL 没有 Python callback；可从 JSON 重建。只在显式请求时生成 rows.json 或量子模块。
...
实现见 [quantum.py](../src/pyqecclang/qham/quantum.py) 和 [stencils.py](../src/pyqecclang/qham/stencils.py)。
...
```python
from pyqecclang.qham import Grid, Discretization, structured_fd_bindings, qham_input_model
...
如果选择稀疏输入型 QODE solver，需要另外提供满足其约定的基础稀疏访问及对应可逆实现。尤其强迫注入可产生稠密列；“行规则很稀疏”并不保证满足 CKS 的双边稀疏假设。这与 [QFVM 的输入模型审查](qfvm-qlss-input-model-review.md) 是同一条设计原则。
...
```bash
# 内置案例
python -m pyqecclang.qham --example burgers --order 3 \
  --eta=-0.4 --state-width 2 -o out/qham-general/my-derivation

# 任意符合 PDE 0.1 Schema 的输入
python -m pyqecclang.qham my-pde.json --order 4 \
  --eta=-0.6 --state-width 3 -o out/my-qham

# 大阶数只查询一个块行，不枚举整个系统
python -m pyqecclang.qham my-pde.json --order 20 \
...
真实 PySparQ 已执行多分量非线性端口、含强迫的二阶生成元和完整的小型 QHAM→有限 Taylor QODE→物理输出链；实际 OriginIR 解析器消费了组合描述。Schrödingerization 和经过显式耗散移位的 CBMD 也已完成输入组装。证据见 [验证记录](qham-general-validation.json)。
