...
本指南面向第一次接触 pyqecclang 的开发者，用九个可以完整运行的案例串起整门语言：从最小的 QRAM 查询模块，一直到从偏微分方程自动推导量子线性系统。每个案例的代码与输出都在当前版本（0.6.0）上实际运行过；涉及算法数学正确性的地方，文中会如实标注哪些已经验证、哪些仍在待核验清单上。规范性的完整定义见 [pyqecclang-spec.md](pyqecclang-spec.md)（下称《规范》），本指南按其章节编号引用。

专门实现或替换微分方程求解器时，配合阅读 [QPDE/QODE 实作指南](qpde-qode-howto.md)。该指南以 LCHS、Schrödingerization 和 Carleman 说明多种 input paradigm，并提供 11 组可运行的 oracle 绑定与 protocol 替换示例。
...
from pyqecclang.algorithms.elementary import deutsch_jozsa
from pyqecclang.oracles import abstract_database, gate_database
...
from pyqecclang.algorithms.elementary import grover
from pyqecclang.oracles import phase_marks
...
这个案例展示库里最深的一条管线：从 QRAM 里的原始流场出发，组装 frozen-Roe 线性系统，然后在两种量子线性求解器协议之间替换。完整审查记录在 [qfvm-qlss-input-model-review.md](qfvm-qlss-input-model-review.md)，工具 `tools/build_qlss_comparison.py` 一键生成两份对照产物。
...
from pyqecclang.qfvm import roe_qfvm_inputs, roe_qfvm_problem
from pyqecclang.algorithms.cks import CKSConfig, make_cks_qlss        # 消费稀疏输入
from pyqecclang.algorithms.costa import CostaConfig, make_costa_qlss  # 请求块编码输入
...
from pyqecclang.qham import (
    Discretization, Field, Grid, Known, PolynomialPDE,
    QHAMPlan, qham_input_model, structured_fd_bindings,
)
...
```bash
PYTHONPATH=src python3 -m pyqecclang.qham --example burgers --order 2 --row physical
```
