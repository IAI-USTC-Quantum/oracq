...
<a href="../../en/development/validation-coverage.html">English</a> · **简体中文**
...
| `vtaa_cks.py` | [VTAA-CKS](../manual/algorithms/vtaa-cks.md) | C2 | `tests/core/test_vtaa_cks.py:VTAAStructureTests` | `GappedPhaseEstimationTests.test_marker_circuit_matches_qsp_response_exactly` / `BandInverseTests.test_band_inverse_step_matches_chebyshev_polynomial` / `VTAAEndToEndTests.test_uniform_spectrum_single_band_is_exact` / `test_variable_time_clock_separates_bands` | `VTAAEndToEndTests` 的日程无关性见证（放大轮数不改变条件解态） | 频带多项式精度标定与过渡带复振幅对拍；VTAA 日程的振幅估计通道 | V3 |
