...
API 入口：{obj}`abstract_prepare <oracq.algorithms.common.prepare_select.abstract_prepare>`、{obj}`gate_prepare <oracq.algorithms.common.prepare_select.gate_prepare>`、{obj}`qram_prepare <oracq.algorithms.common.prepare_select.qram_prepare>`、{obj}`select_pauli <oracq.algorithms.common.prepare_select.select_pauli>`

- `coefficients`：至少两个有限复系数、不全为零（单项请直接用 {obj}`scale <oracq.algorithms.input_model.operators.scale>`）。
- {obj}`abstract_prepare <oracq.algorithms.common.prepare_select.abstract_prepare>`：开放声明，系数写入声明属性，`work_width` 须与后续绑定实现的 work 宽度一致（gate 为 0，QRAM 为 selector 位宽加角度位宽）。
- {obj}`qram_prepare <oracq.algorithms.common.prepare_select.qram_prepare>`：返回 {obj}`QramPreparation(preparation, memory) <oracq.algorithms.common.prepare_select.QramPreparation>`，角表 `memory = {"angles": ...}` 由调用方作为 QRAM 数据绑定（`angle_width` 2..32）。
- {obj}`select_pauli <oracq.algorithms.common.prepare_select.select_pauli>` / {obj}`lcu_prepare_select <oracq.algorithms.common.prepare_select.lcu_prepare_select>` 的 `terms`：`(系数, Pauli 字)` 序列或 {obj}`PauliHamiltonian <oracq.algorithms.common.hamiltonian.PauliHamiltonian>`；字仅允许 `IXYZ` 且各项等宽，SELECT 至少两项。
- `lcu_prepare_select`：`prepare` 缺省 {obj}`gate_prepare <oracq.algorithms.common.prepare_select.gate_prepare>`，也可传 abstract / QRAM / alias 句柄（经 {obj}`as_state_preparation <oracq.algorithms.input_model.interfaces.as_state_preparation>` 适配）；单项退化为 `scale(c, pauli_word)`。

返回对象属性：

| 对象 | 属性 |
|---|---|
| PREPARE 各版（{obj}`StatePreparation <oracq.algorithms.input_model.oracles.StatePreparation>`） | `width` = selector 位宽；模块属性 `prepare_terms` / `prepare_alpha` / `selector_width` |
| `select_pauli`（{obj}`Operation <oracq.infrastructure.builder.Operation>`） | 寄存器 `selector` / `target`；属性 `algorithm="select_pauli"`、`select_terms`、`selector_width` |
| `lcu_prepare_select`（{obj}`BlockEncoding <oracq.algorithms.input_model.operators.BlockEncoding>`） | `be_alpha` = α；属性 `be_form="prepare_select"`、`lcu_terms`、`selector_width`、`prepare_work` |

## 实现要点

gate 版直接复用 {obj}`gate_state_prep <oracq.algorithms.input_model.oracles.gate_state_prep>` 的多路复用 Ry 旋转树（幅度 $\sqrt{|c_i|/\alpha}$ 补零到 $2^{\text{selector}}$）；QRAM 版复用 {obj}`qram_state_prep <oracq.algorithms.input_model.oracles.qram_state_prep>` 旋转树，角表由 {obj}`qram_state_angles <oracq.algorithms.input_model.oracles.qram_state_angles>` 生成。块编码的 signal 寄存器布局：低 `selector_width` 位是 selector，高位是 PREPARE 的 work（alias 版为纯化 junk）；PREPARE 需满足 `zero_input` 契约且目标宽度与 selector 位宽一致，否则生成期拒绝。
...
- 数值：`test_select_applies_indexed_pauli_word_with_phase`——SELECT 逐 selector 值与稠密 Pauli 矩阵 × 系数相位逐元对拍（places = 11）；`test_block_encoding_recovers_hamiltonian_over_alpha`——BE 的 (0,0) 块乘 α 后与稠密哈密顿量对拍（places = 10，经 `witness.assert_block_equals`）；`test_qram_prepare_block_encoding_approaches_hamiltonian`（QRAM PREPARE 版 delta = 0.02）；`test_qubitization_walk_composition`（直接接 {obj}`qubitization_walk <oracq.algorithms.common.transforms.qubitization_walk>`，signal = 0 块 = H/α，places = 10）。
- 绑定：`test_gate_and_qram_prepare_bindings_agree`——gate 版 selector 幅度与 $\sqrt{|c|/\alpha}$ 对拍（places = 11），abstract 槽经 {obj}`bind <oracq.infrastructure.linking.bind>` 绑定 gate 实现后分布精确不变，QRAM 绑定经 {obj}`Binding <oracq.infrastructure.linking.Binding>` 挂角表后幅度对拍（delta = 0.02）；abstract → gate 在块编码内部的绑定由 `test_abstract_prepare_binds_inside_block_encoding` 覆盖（绑定后块不变，places = 10）。
...
- 源码：`src/oracq/algorithms/common/prepare_select.py`
