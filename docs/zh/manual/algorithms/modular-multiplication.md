...
API 入口：{obj}`modular_multiply <oracq.algorithms.basics.number_theory.modular_multiply>`
...
返回 {obj}`Operation <oracq.infrastructure.builder.Operation>`，寄存器为 `target: Bits(width)`。模块属性：
...
- 结构：`tests/core/test_algorithm_expansion.py:AlgorithmExpansionTests` 的构造与属性断言；`test_bad_inputs_fail_at_generation` 覆盖乘数与模数不互素（{obj}`modular_multiply(2, 4) <oracq.algorithms.basics.number_theory.modular_multiply>`）等生成期违例。
