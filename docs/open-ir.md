...
本文是 RIR 0.2 相对于 rir-spec.md 中 0.1 的规范增量。版本 0.2 同时读取原有 0.1 程序。生成器默认输出 0.2。
...
这一版开放的是实现主体，签名中的宽度和用于组装的常量仍然具体。改变 BE 的 M 或 alpha，需要重新运行相同的 Python 生成器，得到对应的新 IR。链接器不会在已经生成的 LCU 角度里偷偷替换 alpha。
