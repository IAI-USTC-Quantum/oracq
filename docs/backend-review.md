...
| DEF 中的 whole-register 实参 | 本轮最小实验中，嵌套调用 load(a,d) 被解析器拒绝。 | 导出 load(a[0],d[0],...)，保留模块调用。 |
...
本轮遇到的解析错误会被 Simulator 的格式自动回退包装成 OpenQASM 寄存器错误。调试时应直接检查 OriginIR_BaseParser，而不能只根据最终异常判断源文本类型。
...
本轮 33 个绑定案例通过真实 OriginIR-ext 解析。没有以数值结果作为本阶段验收条件。
