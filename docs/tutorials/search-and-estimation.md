...
标准[振幅估计](../manual/algorithms/qae.md)对 Grover iterate 做相位估计。它接收初态制备和好状态集合，输出 phase 寄存器。下面的初态在 `0` 和 `1` 上均匀分布，故好状态 `1` 的概率为 `1/2`。
...
打印出的字典把两个相位字解码回概率：`2` 和 `6` 都给出约 `1/2`，未舍入的解码值与 `1/2` 的差异在浮点末位，来自有限精度读出的舍入。一般情况下，有限精度的相位读出会产生近似估计；多次采样和统计处理在宿主侧进行。
