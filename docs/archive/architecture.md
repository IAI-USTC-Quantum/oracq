...
RIR 0.3 的私有工作区见 [rir-spec.md](../reference/rir.md)；0.2 的开放声明和绑定增量见 [open-ir.md](../reference/open-ir.md)。ir.py 定义不可变记录。validation.py 检查存储类型、寄存器视图、别名、控制位、资源参数、操作元数和调用图。serialization.py 提供严格的 JSON 往返。
...
Python 纯函数经 MIR 0.1 转换到既有 RIR 0.3。MIR 保存类型、SSA 节点和 helper 调用，生成配置决定定点格式与数学核近似；helper 调用和量子模块边界保留。自动 lowering 负责输出 XOR 与私有工作区反算。完整使用方式见 [纯函数编译](../manual/math-functions.md)，对象契约见 [MIR 规范](../reference/math-ir.md)。
