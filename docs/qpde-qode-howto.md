...
本文对齐 pyqecclang 0.6 的实际接口。目标是让你能写出自己的 PDE 离散化、替换输入 oracle，再选择 LCHS、Schrödingerization 或 Carleman 组合成程序。完整可运行文件是 [examples/ode_input_models.py](../examples/ode_input_models.py)；本文中的局部片段用于解释接口，完整脚本包含导入、数据、绑定和导出。
...
本轮补充使用说明与示例，不改变 RIR 或核心求解器。以下明确区分已有接口、教程中的宿主适配函数和未来扩展。结构可组装不等于论文的误差、成功概率或复杂度结论已经成立。
...
两个 QPDE 工厂的调用约定不同。`make_qpde` 在 [solvers.py](../src/pyqecclang/algorithms/solvers.py)，其他主要入口在 [differential.py](../src/pyqecclang/algorithms/differential.py)。不要直接把三参数 `linear_qode(...)` 传给默认的 `qpde_solver`。
...
from pyqecclang.oracles import StateOracle, StatePreparation
...
这是文档中的类型示意；包内目前没有名为 `LinearSolver` 的统一 QODE 类。已有 QLSS 的 `LinearSystem/QLSSProtocol/SolveResult` 是另一套接口，不能据此假定 QODE 也已有自动 input-model 分派或完整的范数恢复对象。
...
from pyqecclang.oracles import abstract_database, diagonal_block_encoding, qram_database
...
from pyqecclang.oracles import abstract_sparse_access
from pyqecclang.sparse_models import real_symmetric_sparse_encoding
...
一般复数/非对称稀疏矩阵需要另一套适配。尤其不能为了适配 QODE 就直接把 `G` 替换成 `[[0,G],[G†,0]]` 并演化：后者的指数没有自动等于 `exp(tG)` 的物理输出块。QLSS 的 Hermitian dilation 技巧不等于 QODE 的动力学变换。相关输入区别见 [QFVM/QLSS 审查](qfvm-qlss-input-model-review.md)。
...
```python
from pyqecclang.combinators import lcu
from pyqecclang.algorithms.solvers import apply_be_to_state
...
```python
from functools import partial
from pyqecclang.algorithms.differential import QuadraturePlan, linear_qode, taylor_hamiltonian
...
若已有仅接受 Pauli 列表的 Trotter 实现，它不能从任意黑盒 BE 自动恢复 Pauli 分解。应在上层保留该结构，写显式适配器或另一个 solver 生成器。`solvers.py` 中早期的 `make_lchs_qode(ham_sim,times,weights)` 只是调用方自备分解的 LCU 框架，未构造一般 `H+kL`；新算法应使用本文入口。
...
from pyqecclang.algorithms.differential import (
    SchrodingerPlan, linear_qode, taylor_hamiltonian,
)
...
`solvers.py` 中历史入口 `make_schrodingerisation_qode(embedding,ham_sim,...)` 由调用者自行负责 embedding，未自动完成这里的 warped 初态/QFT；新实现以 `differential.py` 为准。
...
```python
from pyqecclang.qham import Discretization, Field, Grid, PolynomialPDE, structured_fd_bindings
...
from pyqecclang.combinators import lcu
from pyqecclang.algorithms.differential import PolynomialODE
...
from functools import partial
from pyqecclang.algorithms.differential import (
    PDEInput, carleman_qode, linear_qode, qpde_solver, taylor_hamiltonian,
)
...
这与 QHAM 的一般有限闭包不同：Carleman 丢弃张量尾项；QHAM 的 QCL 表示针对已选 HAM 截断建立闭包。二者可以共享 `F_p` 与线性 QODE protocol，但截断含义不能混同，见 [QHAM 数学推导](qham-general-derivation.md)。
...
from pyqecclang.qham import Grid
from pyqecclang.qham.stencils import derivative_encoding
from pyqecclang.algorithms.solvers import DiscretePDE, make_qpde
...
这符合“语言负责可组装、精度由生成函数和应用负责”的设计。但应用必须能找到这些契约：建议每个 solver 配套宿主结果记录，而不是靠零散 metadata 推断数学结论。该建议是下一步接口整理方向，本轮没有引入一个假装完整的统一 solver 类。
...
本轮 11 个案例均通过 RIR 序列化往返、闭合检查、两种 OriginIR 导出，并用真实 `OriginIR_BaseParser` 解析严格门集产物。此验证确认描述可消费，不证明有限 Taylor、积分截断、Fourier 恢复或 Carleman 的数值求解正确性。原始模块调用在 pyqecclang RIR 和导出的 DEF 中保留；下游解析器内部仍会展开 DEF。

`--native-bindings` 只比较 `given_xor_gate_lchs` 与 `given_xor_qram_lchs` 的完整复幅度，记录为 `binding-validation.json`。其他案例并不因此获得量子态正确性结论。稀疏 gate-LCHS 案例展开后有 1,936,453 个执行事件，超过 PySparQ 适配器默认一百万步预算；本轮对它完成描述与解析验证，没有通过扩大模拟范围来代替算法验证。

本轮实际角数据库绑定对拍的最大复幅度差为 0；`tests/core/test_differential.py` 的 4 个测试（含 3 个 subtest）通过，`ruff check src tests examples tools` 通过。文档链接和 Python 片段语法已检查。

继续阅读：[oracle 目录](oracle-paradigms.md)、[开放绑定规范](open-ir.md)、[一般 QHAM 实现](qham-general-implementation.md)、[数学函数编译](function-compiler.md)。
