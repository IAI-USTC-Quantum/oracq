...
`infrastructure` 负责描述的结构与执行机制，不根据算法名字选择线路。数学函数前端可以调用算法库的算术生成器；`Operation` 的便捷视图方法也会在需要时调用算法适配器。这些调用发生在 Python 生成阶段，不增加 RIR 指令种类。

可选后端在执行入口导入。生成与序列化 RIR 不要求安装 PySparQ 或 UnifiedQuantum。

## 算法库

`algorithms` 中每个类别有自己的文件和输入约定。共享的 `interfaces.py` 提供常见 Python 结构协议；`contracts.py` 提供检查报告。应用仍可定义自己的协议。

`operators.py` 保存 BE 包装类与基本缩放操作，`block_encoding.py` 提供 LCU、张量和小矩阵构造。它们是算法的组合工具。`ode.py` 提供可替换线性求解接口；LCHS、Schrödingerization、CBMD 和 Carleman 各自维护实现。

## 领域应用

QFVM 的物理参数、Roe 公式和经典数据更新放在 `applications`。QHAM 的量子组装入口是 `algorithms/qham.py`；PDE 表达式、同伦推导和空间离散化支持放在 `applications/qham/`。这样可以阅读量子生成步骤，而不必同时展开全部数学推导代码。
