...
量子信号处理（QSP）相位合成：给定实系数目标多项式，求出使 [QSVT 相位序列](qsvt-sequence.md) 实现该多项式的相位序列 $\Phi$。模块导出一对互逆例程：`qsp_response(x, phases)` 正向求值，返回相位序列在反射约定下的顶层左块 $p(x)$（$x \in [-1, 1]$）；`qsp_phases(coeffs, imag=None)` 逆向合成，返回时间正序相位（长度 $d + 1$，$d$ 为目标度数）。
...
两个入口都是纯数值例程，不生成量子程序，因此没有 input model；产物（相位元组）供 `qsvt_sequence` 与标准变换族消费。可实现条件（违反时抛 `ValidationError`）：$f$ 与 $h$ 的奇偶性均为 $d \bmod 2$；$[-1,1]$ 上 $f^2 + h^2 \le 1$；端点饱和 $f(\pm 1)^2 + h(\pm 1)^2 = 1$；补多项式 $R = (1 - f^2 - h^2)/(1 - x^2)$ 非负且满足谱分解的根重数条件。$d = 0$ 的目标必须是单位模常数，此时返回单相位。
...
`qsp_response` 的求值是 2×2 矩阵递推：从恒等阵出发，每步先左乘 $W(x)$（时间上先于本步相位的作用，首相位除外）、再左乘 $S(\varphi)$，返回累积矩阵的 $(0,0)$ 元。除独立调用外，它还是合成器内部的自检工具：`qsp_phases` 与 `fixed_point_search_phases` 在剥离结束后都用它做网格往返校验。
