...
定义三个可组合构件：问题对象 `QODEProblem`（生成元、初态、耗散声明与证据说明）、可替换协议 `QODEProtocol`（契约检查与声明透传），以及通用入口 `linear_qode`，把 $G$ 路由到 LCHS、CBMD 或 Schrödingerization 等具体方法。另提供 `make_euler_history_qode`，把隐式 Euler 时间推进写成一次量子线性系统求解。
...
- `generator`：$G$ 的 `BlockEncoding`（input model 为 BE），需具备 adjoint/controlled 能力。
- `initial`：`StatePreparation`（SP），零输入、干净 work，宽度与 `generator` 相同。
...
| `__call__(generator, initial, time)` | 兼容入口：check 通过后生成，返回 `StateOracle` |
...
`QODEProtocol.contract` 在共享的 `operator_state_contract` 之上按方法附加假设：lchs/cbmd 追加 "Hermitian(G)<=0；问题级 solve 必须声明 dissipative=True"，schrodingerization 追加辅助窗口、Fourier 约定与恢复区域的应用层验证责任。`_generate` 强制输出为 `StateOracle`、宽度与输入一致且保持 adjoint/controlled 能力，违例在生成期抛 `ContractError`。lchs/cbmd 路线内部先构造 `LinearODE(HermitianParts.from_operator(scale(-1, generator)), initial)`，即 $A=-G$ 的 $L+iH$ 分解；schrodingerization 直接消费 $G$。Hermitian/半正定性质均由调用方承担。
...
其中 `q = projector(nt, range(1, steps + 1))` 把 $G$ 限制在时间槽 $1..\text{steps}$，`S = truncated_shift(nt, steps)` 是槽间提升移位；展开后每个槽的方程恰为隐式 Euler $(I-\Delta t\,G)\,u_k = u_{k-1}$，槽 0 由 `extend_initial` 填入初态。`qlss(c, rhs)` 求解后经 `select_subspace` 选出时间槽 `steps` 的快照，选择条件并入 signal。
