...
对任意态 oracle 的成功子空间做相干放大：设 `StateOracle` 的制备 $A$ 以概率 $a$ 产出成功态（成功条件 signal==0），迭代算子
...
- `state`：支持伴随调用的 `StateOracle`（`target`/`signal` 接口的态 oracle；signal 指示成功子空间，属 FO 幅度型访问）。非 `StateOracle` 输入在生成期抛 `ValidationError`。
...
先正向调用一次 $A$ 制备初态；随后每次迭代为 `reflect_zero(signal)`（对 signal==0 的好子空间相位翻转，充当 $S_{\mathrm{good}}$）→ $A^\dagger$ → 关于 target+signal 全零态的正反射 → $A$。与 `grover_iterate` 的算子形状一致，差别只在标记来源：这里 $S_{\mathrm{good}}$ 由 signal 寄存器本身承担，因此适用于任意来源的成功指示（如 QRAM 查询结果的判定位、算法工作区的检验位）。迭代以 `repeat(iterations)` 保存，不在生成或 JSON 序列化阶段展开。
...
- 结构：`tests/core/test_algorithm_expansion.py:AlgorithmExpansionTests` 的构造与属性断言（`require_instance` 校验 StateOracle、迭代次数非负）。
