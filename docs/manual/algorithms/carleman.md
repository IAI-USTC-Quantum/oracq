...
- `initial` / `initial_norm`：物理初态的 `StatePreparation`（SP）与原始向量范数；范数决定提升初态各阶的相对幅值，不是装饰属性。
- `linear_solver`：三参数协议 `(generator, initial, time) -> StateOracle`，通常注入 `linear_qode(...)`。

三个入口分别返回：提升生成元 $G_K$ 的 `BlockEncoding`（属性 `algorithm="carleman_lift"`、`cutoff`、`coefficient_assumption`）、提升初态的 `StatePreparation`、以及最终 `StateOracle`（属性 `algorithm="carleman_qode"`、`cutoff`、`truncation_assumption="Carleman tail pending"`）。

## 实现要点

寄存器布局为补齐布局：`target = 数据区（K·n 位，低）+ level 寄存器（K.bit_length() 位，高）`；阶 $k$ 的有效数据只占前 $k\cdot n$ 位，其余为零——不是紧凑的 $1+d+\dots+d^K$ 地址，资源差别保留在报告中。

`carleman_lift` 对每个 $(k,p,\text{position})$（$1\le k\le K$、来源阶 $k+p-1\le K$、位置 $0..k-1$）生成一个放置 BE：level 寄存器从来源阶路由到输出阶（XOR 翻转差位），$F_p$ 作用在 position 起的连续 $p$ 个 $n$ 位组上，$p>1$ 时输出保留在组内最低 $n$ 位、零行移到数据区高端；所有项 LCU 求和得 $G_K$。

`carleman_initial` 制备归一化提升向量

$$
\frac{(1,\,u_0,\,u_0^{\otimes 2},\,\dots,\,u_0^{\otimes K})}{\sqrt{\sum_{k=0}^{K}\lVert u_0\rVert^{2k}}},
$$

按 level 受控地多次调用初态 oracle（work 预留 $K\times$ 初态 work 宽度）。`carleman_qode` 把 $G_K$ 与提升初态交给注入的线性求解器，再以 `select_subspace` 选中 level 1 通道（选择值 `1 << ((cutoff-1)*width)` 恰落在 level 寄存器最低位，同时要求高位数据位为零），选择条件并入 signal。
...
- 结构：`tests/core/test_differential.py:DifferentialStructureTests.test_four_methods_keep_input_oracles` 的 Carleman 分支——`PolynomialODE(1, ((1, a), (2, f2)), initial)` 以抽象 BE 为系数、CBMD 为线性求解器时，开放槽 `input_A`/`input_b`/`input_F2` 全部保留，且存在 `carleman_row_level == 1`、`carleman_column_level == 2` 的放置模块，序列化往返成立。
- 数值：`tests/core/test_differential.py` 定位为结构测试；截断收敛的数值对拍归入统一的 ODE 收敛基准（见已知缺口）。
- 绑定：同类注册与契约检查——系数 BE 宽度、次数唯一性与初态布局在 `PolynomialODE.__post_init__` 生成期校验，违例抛 `ValidationError`。
