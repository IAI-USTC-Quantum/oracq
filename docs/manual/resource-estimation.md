...
> 模块 `pyqecclang.infrastructure.estimate` 与 `pyqecclang.infrastructure.backends.strict`
...
组合式的：按 `(module, 控制数)` 记忆化聚合，`Repeat` 以符号计数相乘——
`2^40` 次重复的程序不需要展开即可估计，这正是 RIR「结构保持」设计直接
买来的能力。

同一降低链路上的 `export_strict(program)` 给出**可检查的编译产物**：模块
...
零值控制位在每个发射行组两侧各翻转一次；每次 `Load` 记 1 次 QRAM 查询。
...
- 估计器按**当前编译器的发射配方**计数：常量加法走 O(n³) 阶梯（文献有
  O(n) 可逆加法器）、`gate_database`/`qrom_lookup` 走逐分支真值表网络
  （理想化 unary-iteration QROM 为 N−1 Toffoli，见
  `qrom_cost` 模型与 babbush2018encoding）。真实发射与理想化模型之间的
...
| `qft` | n=3..12 | 旋转 3→165，精确 T 6→33 | ≈2.85（π/4 网格精确旋转在小 n 处掉出） |
| `qpe_add_const` | p=3..12 | p=12 时门数 ~5.5万 | 符号 Repeat：~2^p × cost(add) |
| `grover` | n=4..14 | 迭代 ⌊π/4·2^{n/2}⌋ | 总门数 ~2^{n/2}·O(n) |
...
| `roe_face` / `math_polynomial` | w=6..16 | Roe 面 w=16 时 Toffoli 446 076 | ≈1.8，字长多项式 |
